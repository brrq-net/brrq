...
import { ArrowRightLeft, ArrowRight, Blocks } from 'lucide-react';
import { useRecentBlocks } from '@/hooks/useBlocks';
import { timeAgo, shortenHash } from '@/lib/utils';
import { LoadingSpinner } from '@/components/ui/LoadingSpinner';

/** Shows recent blocks that contain transactions. */
export function RecentTxs() {
  const { data: blocks, isLoading, isError } = useRecentBlocks(10);

  // Filter to blocks with at least one transaction
  const blocksWithTxs = blocks?.filter((b) => b.tx_count > 0).slice(0, 5);
    ...
    <div className="bg-[var(--obsidian)] cyber-card gold-border gold-glow">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--gunmetal)]">
        <h2 className="text-lg font-semibold text-[var(--text-primary)] flex items-center gap-2">
          <ArrowRightLeft className="h-5 w-5 text-[var(--brrq-gold-light)]" />
          Recent Activity
        </h2>
        <Link
          href="/txs"
      ...
      <div className="divide-y divide-[var(--gunmetal)]/50">
        ...
        {blocksWithTxs && blocksWithTxs.length === 0 && (
          <div className="px-5 py-8 text-center text-[var(--muted-silver)]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Link
                      ...
                      className="text-[var(--brrq-gold-light)] hover:text-[var(--brrq-gold)] font-medium text-sm"
                    >
