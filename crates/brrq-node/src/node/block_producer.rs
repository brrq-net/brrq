...
fn compute_gas_revenue(txs: &[Transaction], per_tx_gas: &[u64]) -> u64 {
    txs.iter()
        .zip(per_tx_gas.iter())
        .map(|(tx, &gas_used)| {
            (tx.body.max_fee_per_gas as u128)
...
fn distribute_fees(
    ...
    let total_gas_revenue = if gas_used > 0 {
        compute_gas_revenue(txs, per_tx_gas)
    } else {
        0
    };
...
async fn generate_batch_proof(shared: SharedState) {
    // Read batch parameters and collect pending traces from shared state
    let (initial_root, final_root, block_range, tx_count, total_gas, event_tx, batch_trace) = {
        let ns = shared.write().await;
        let batch_size = ns.batch_proof_config.batch_size;
        ...
        let batch_trace = brrq_vm::trace::ExecutionTrace::with_capacity(0);
