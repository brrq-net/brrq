...
impl NetworkService {
    ...
    async fn bootstrap_and_discover_loop(
        ...
        // When peer_count drops to 0 (network partition), re-attempt bootstrap connections
        // to recover. Without this, a partitioned node stays partitioned forever.
        let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(30));
        loop {
            interval.tick().await;

            let conn_count = connections.read().await.len();
            if conn_count == 0 && !bootstrap_nodes.is_empty() {
                tracing::warn!(
                    "Zero peers connected — attempting bootstrap reconnection ({} nodes)",
                    bootstrap_nodes.len()
                );
                for addr in &bootstrap_nodes {
                    ...
                    let mut stream = match TcpStream::connect(addr).await {
                        Ok(s) => s,
                        Err(e) => {
                            tracing::debug!("Bootstrap reconnect to {} failed: {}", addr, e);
                            continue;
                        }
                    };
                    if let Err(reason) = Self::send_hello_with_timeout(
                        &mut stream, &node_id, &network, &shared,
                        node_secret_key.as_ref(), addr,
                    ).await {
                        tracing::debug!("Bootstrap hello to {} failed: {}", addr, reason);
                        continue;
                    }
                    let peer_id = format!("bootstrap_{}", addr);
                    peers.write().await
                    ...
                    tracing::info!("Reconnected to bootstrap node: {}", addr);
                }
            }
