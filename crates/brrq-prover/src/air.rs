...
pub const TRACE_WIDTH: usize = 152;
...
///   = 7 per row × 2 = 14
pub const NUM_TRANSITION_CONSTRAINTS: usize = 302; // 288 + 14 (Phase 6)
...
fn evaluate_transition_generic<F: AirField>(current: &[F], next: &[F]) -> Vec<F> {
    ...
    constraints.push(next[COL_PC].sub(current[COL_NEXT_PC]));
    ...
    let is_sequential = F::ONE
        ...
        constraints.push(is_sequential.mul(next_pc_minus_pc4));
        ...
        // ════════════════════════════════════════════════════════════════
        // REGISTER INVARIANTS (2 constraints: T5-T6)
        // ════════════════════════════════════════════════════════════════
...
mod tests {
    ...
    fn test_invalid_x0_current() {
        ...
        let constraints = air.evaluate_transition(&frame);
        assert_ne!(constraints[4].value(), 0, "T5: current x0 should fail");
    }
    ...
    fn test_invalid_x0_next() {
        ...
        let constraints = air.evaluate_transition(&frame);
        assert_ne!(constraints[5].value(), 0, "T6: next x0 should fail");
    }
    ...
    fn test_selector_sum_violation() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // T27 (index 26): selector sum constraint (current).
        assert_ne!(
            constraints[26].value(),
            0,
            "T27: selector sum ≠ 1 should fail"
        );
    }
    ...
    fn test_selector_non_boolean() {
        ...
        // is_alu_imm is at SELECTOR_COLUMNS[6], so T7+6 = T13 (index 12).
        let bool_idx = 6 + 6; // T7 starts at index 6, is_alu_imm is 6th in SELECTOR_COLUMNS
        assert_ne!(
    ...
    fn test_opcode_bit_non_boolean() {
        ...
        assert_ne!(constraints[28].value(), 0, "opcode bit boolean should fail");
    }
    ...
    fn test_opcode_reconstruction_mismatch() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // T43 (opcode reconstruction, current) at index 42.
        assert_ne!(
            constraints[42].value(),
            0,
    ...
    fn adversarial_forged_selector() {
        ...
        // T45-T54: current row binding. is_jal is SELECTOR_COLUMNS[2] → T45+2 = T47.
        let binding_base = 44; // T45 starts at index 44
    ...
    fn test_num_constraints() {
        ...
        assert_eq!(air.num_constraints(), 305); // 302 transition + 3 boundary (Phase 6: +14 carry/MULH/mem)
    }
    ...
    fn test_trace_width() {
        ...
        assert_eq!(air.trace_width(), 152); // Phase 1-5 (138) + Phase 6 (14: carry decomp 8 + MULH 4 + mem 2)
    }
    ...
    fn adversarial_no_selector_active() {
        ...
        frame.current[COL_IS_ALU_IMM] = Fp::ZERO;
        let constraints = air.evaluate_transition(&frame);
        // T27: selector sum should be -1 (sum=0, constraint=0-1=-1).
        assert_ne!(
            constraints[26].value(),
            0,
    ...
    fn adversarial_tampered_addi_result() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // T123: is_addi * (rd_val_after - rs1_val - imm) = 1*(42-0-0) = 42 ≠ 0
        assert_ne!(
            constraints[122].value(),
            0,
    ...
    fn adversarial_forged_sub_selector() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // T92: is_add * (1 - is_alu_reg) = 1*(1-0) = 1 ≠ 0
        assert_ne!(
            constraints[91].value(),
            0,
    ...
    fn adversarial_funct3_reconstruction() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // T89: funct3 reconstruction mismatch
        assert_ne!(
            constraints[88].value(),
            0,
    ...
    fn adversarial_wrong_add_result() {
        ...
        // T124: is_add * (rd_val_after - rs1_val - rs2_val) = 1*(99-10-25) = 64 ≠ 0
        assert_ne!(constraints[123].value(), 0, "Wrong ADD result should fail");
    }
    ...
    fn adversarial_sub_selector_boolean() {
        ...
        let constraints = air.evaluate_transition(&frame);
        // is_addi is at position 7 in PHASE1B_BOOLEAN_COLUMNS → T72 (index 71)
        assert_ne!(
            constraints[71].value(),
            0,
