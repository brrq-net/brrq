...
use crate::air::{
    ...
    COL_MEM_STEP, COL_MEM_PREV_VALUE, COL_IS_TX_BOUNDARY, COL_HAS_COPROCESSOR,
};
...
pub fn convert_trace(trace: &ExecutionTrace) -> Result<AlgebraicTrace, ProverError> {
    ...
    // Build set of CPU steps that have actual coprocessor entries.
    // Only these ECALLs should be marked is_ecall=1 for LogUp linking.
    // Internal syscalls (SLOAD, SSTORE, env reads, HALT, etc.) use the
    // ECALL opcode but don't produce coprocessor trace entries — marking
    // them is_ecall=1 would cause LogUp sum mismatch.
    let coprocessor_steps: std::collections::HashSet<u64> = trace
        .coprocessor
        .cpu_steps
        .iter()
        .copied()
    ...
    for (row, step) in trace.steps.iter().enumerate() {
        ...
        // Column 37: ECALL selector — 1 for ALL SYSTEM opcode instructions.
        // This is part of the selector sum constraint (sum of all selectors = 1).
    ...
    // ── Coprocessor-linked ECALL markers ──
    // Only ECALLs with matching coprocessor entries get has_coprocessor=1.
    // This prevents LogUp sum mismatch for internal syscalls (SLOAD, etc.).
    for &cpu_step in &trace.coprocessor.cpu_steps {
        let row = cpu_step as usize;
        if row < real_steps {
            columns[COL_HAS_COPROCESSOR][row] = 1;
        }
    }

    // ── Transaction boundary markers ──
    // Mark rows where a new transaction begins within the batch trace.
    // The STARK AIR relaxes PC flow and register continuity constraints here.
    for &boundary_step in &trace.tx_boundaries {
        let row = boundary_step as usize;
        if row < real_steps {
            columns[COL_IS_TX_BOUNDARY][row] = 1;
        }
    }
