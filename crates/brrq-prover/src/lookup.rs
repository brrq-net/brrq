...
use crate::air::{
    ...
    COL_RD, COL_RD_BYTE_0, COL_RD_VAL_AFTER, COL_RS1, COL_RS1_BYTE_0, COL_RS1_VAL,
    COL_RS2, COL_RS2_BYTE_0, COL_RS2_VAL, COL_SHIFT_AUX_BYTE_0,
};
use crate::field::Fp;
...
impl RegisterFileTable {
    ...
    /// columns (is_write, rd, rd_val_after) and builds its own state.
    pub fn from_trace(trace: &AlgebraicTrace) -> Self {
        let mut table = Self {
            write_log: vec![vec![]; 32],
            write_count: [0u32; 32],
            current_state: [0u32; 32],
        };
    ...
    /// Look up the value of register `reg_idx` at cycle `cycle`.
    ///
    /// Returns the value from the most recent write to `reg_idx` that
    /// happened at or before `cycle`. Returns 0 if no write has occurred
    /// (initial state) or if reg_idx is x0 (hardwired zero).
    pub fn lookup_at_cycle(&self, reg_idx: usize, cycle: u32) -> u32 {
        if reg_idx == 0 || reg_idx >= 32 {
            return 0;
        }
        // Binary search for the last write at or before `cycle`.
        let log = &self.write_log[reg_idx];
        match log.binary_search_by_key(&cycle, |&(c, _)| c) {
            Ok(idx) => log[idx].1,
            Err(0) => 0, // no write before this cycle
            Err(idx) => log[idx - 1].1,
        }
    }
...
pub fn extract_regfile_bus_values(
    ...
    for row in 0..n {
        ...
        if is_write == 1 {
            ...
            // Regfile side: for writes, use the write value from the independent
            // table. Since the table was built from the same write events,
            // this will match for honest provers.
            let independent_val = if rd_idx == 0 {
                0 // x0 is always zero
            } else {
                reg_table.lookup_at_cycle(rd_idx as usize, row as u32)
            };
            let table_bv = regfile_bus_value(
                step,
                Fp::new(rd_idx),
