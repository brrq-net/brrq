...
impl BitcoinRpcClient {
    ...
    pub fn list_since_block(
        ...
        for tx in &transactions {
            ...
            // scriptPubKey is not in listsinceblock output — fetch from
            // getrawtransaction to derive the L2 recipient address.
            let script_bytes = match self.get_tx_script_pubkey(txid_hex, vout) {
                Ok(s) => s,
                Err(_) => Vec::new(), // fallback: will be skipped by deposit_watcher
            };
    ...
    /// Fetch the scriptPubKey for a specific output of a Bitcoin transaction.
    /// Used by list_since_block to derive the L2 recipient address from deposits.
    fn get_tx_script_pubkey(&self, txid: &str, vout: u32) -> Result<Vec<u8>, BitcoinError> {
        let client = self.require_client()?;
        let txid_parsed: bitcoin::Txid = txid.parse()
            .map_err(|e| BitcoinError::RpcCallFailed(format!("invalid txid: {}", e)))?;
        let raw = client.get_raw_transaction_info(&txid_parsed, None)
            .map_err(|e| BitcoinError::RpcCallFailed(format!("getrawtransaction failed: {}", e)))?;
        let output = raw.vout.get(vout as usize)
            .ok_or_else(|| BitcoinError::RpcCallFailed(format!("vout {} not found", vout)))?;
        let script = output.script_pub_key.script()
            .map_err(|e| BitcoinError::RpcCallFailed(format!("script decode: {}", e)))?;
        Ok(script.to_bytes())
    }
