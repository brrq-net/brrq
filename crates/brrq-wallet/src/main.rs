...
enum Commands {
    ...
    CreateAccount {
        ...
        /// Overwrite existing keyfile (DANGEROUS — destroys the old key permanently).
        #[arg(long, default_value_t = false)]
        force: bool,
    },
...
async fn main() {
    ...
    match cli.command {
        Commands::CreateAccount { output, plaintext, force } => cmd_create_account(&output, plaintext, force),
        Commands::ImportKey {
            secret,
            output,
            plaintext,
...
fn cmd_create_account(output: &str, plaintext: bool, force: bool) {
    // Protect against accidental overwrite of existing keyfiles.
    // A lost keyfile means lost funds — this is irreversible.
    if !force && std::path::Path::new(output).exists() {
        eprintln!("Error: keyfile already exists at '{output}'");
        eprintln!("  Overwriting would PERMANENTLY DESTROY the existing key.");
        eprintln!("  If you are sure, use --force to overwrite.");
        std::process::exit(1);
    }
...
fn cmd_import_key(secret_hex: &str, output: &str, plaintext: bool) {
    // Same overwrite protection as create-account
    if std::path::Path::new(output).exists() {
        eprintln!("Error: keyfile already exists at '{output}'");
        eprintln!("  Overwriting would PERMANENTLY DESTROY the existing key.");
        eprintln!("  Delete the file manually first if you intend to replace it.");
        std::process::exit(1);
    }
...
mod tests {
    ...
    fn test_cli_parse_create_account() {
        ...
        match cli.command {
            Commands::CreateAccount { output, plaintext, force } => {
