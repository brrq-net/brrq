...
///
/// Priority order:
/// 1. `X-Real-IP` header (set by nginx `proxy_set_header X-Real-IP $remote_addr`)
/// 2. First entry in `X-Forwarded-For` header
/// 3. Direct `ConnectInfo` socket address (fallback for non-proxied connections)
///
/// Using `ConnectInfo` alone is incorrect behind a reverse proxy — all requests
/// appear as `127.0.0.1`, causing every user to share one rate-limit bucket.
fn extract_client_ip(request: &Request<Body>, socket_ip: IpAddr) -> IpAddr {
    // Prefer X-Real-IP (single trusted proxy hop)
    if let Some(val) = request.headers().get("x-real-ip") {
    ...
    if let Some(val) = request.headers().get("x-forwarded-for") {
        if let Ok(s) = val.to_str() {
    ...
    // No proxy headers — use direct socket address
    socket_ip
}

/// Rate-limits requests per real client IP.
