...
pub fn routes() -> Router<AppState> {
    ...
    // Stub: return empty sequencers list when feature is disabled
    #[cfg(not(feature = "sequencer-rotation"))]
    let router = router.route(
        "/sequencers",
        get(|| async {
            Json(serde_json::json!({ "sequencers": [] }))
        }),
    );
...
/// List recent transactions with pagination (latest first).
///
/// Uses `tx_block_heights` index to only load blocks that contain transactions.
/// Complexity: O(k) where k = number of blocks with txs, not O(chain_height).
/// The index is built from receipts at startup and updated on each new block.
async fn list_transactions(
    ...
    // Iterate only over block heights known to contain transactions (newest first)
        ...
        if let Some(block) = ns.get_block(h) {
            for tx in &block.transactions {
                if skipped < params.offset {
                    ...
                    all_txs.push(tx_to_rest_json(tx, Some(block.header.height)));
                    collected += 1;
                }
            }
        }
...
async fn get_block_latest(
    ...
    match ns.get_block(height) {
        ...
        None => Err(ApiError::NotFound("No blocks yet".into())),
    }
}
...
async fn get_account_transactions(
    ...
    // O(k) where k = blocks-with-txs, not O(chain_height).
    for &h in &ns.tx_block_heights {
        if let Some(block) = ns.get_block(h) {
            for tx in &block.transactions {
                ...
                if is_sender || is_recipient {
                    ...
                    if skipped < params.offset {
                        ...
                        result.push(tx_to_rest_json(tx, Some(block.header.height)));
                    }
                }
            }
        }
