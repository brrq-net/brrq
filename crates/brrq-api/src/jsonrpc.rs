...
async fn handle_get_receipt(
    ...
    let ns = shared.read().await;
    match ns.receipts.get(&hash) {
        Some(r) => RpcResponse::success(
            id,
            serde_json::json!({
...
async fn handle_get_transaction(
    ...
    let ns = shared.read().await;
    // KNOWN PERFORMANCE LIMITATION: O(n*m) linear scan over all blocks and
    // their transactions. The `return` below short-circuits on first match,
    // but this is still O(n) in the worst case (tx not found).
    // Future optimization: maintain a HashMap<Hash256, (u64, usize)> index
    // mapping tx hash -> (block_height, tx_index) for O(1) lookups, updated
    // incrementally as new blocks are appended.
...
async fn handle_get_account_proof(
    ...
    RpcResponse::success(
        ...
        serde_json::json!({
            ...
            "proof": {
                "siblings": siblings,
                "root": format!("0x{}", hex::encode(ns.state.state_root().as_bytes())),
            },
        }),
    )
}
...
async fn handle_get_storage_proof(
    ...
    RpcResponse::success(
        ...
        serde_json::json!({
            ...
            "account_proof": { "siblings": account_siblings },
            "storage_proof": { "siblings": storage_siblings },
        }),
    )
}
...
mod tests {
    ...
    async fn test_params_array_accepted() {
        ...
        let resp = dispatch_single(req, &app).await;
    ...
    async fn test_batch_within_limit() {
        ...
        let resp = handle_jsonrpc(axum::extract::State(app), Json(req)).await;
