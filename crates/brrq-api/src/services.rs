        ...
        "transfer" => {
            ...
            if to.is_zero() {
                return Err(ServiceError::BadRequest(
                    "Transfer to zero address is not allowed".into(),
                ));
            }
...
pub fn submit_to_mempool(ns: &mut NodeState, tx: Transaction) -> Result<String, ServiceError> {
    ...
    // Reject transactions with nonce too far ahead of confirmed account nonce
    let account_nonce = ns.state.nonce(&tx.body.from);
    if tx.body.nonce > account_nonce + brrq_sequencer::mempool::MAX_NONCE_GAP {
        return Err(ServiceError::BadRequest(format!(
            "nonce {} too far ahead of account nonce {} (max gap {})",
            tx.body.nonce, account_nonce, brrq_sequencer::mempool::MAX_NONCE_GAP
        )));
    }
