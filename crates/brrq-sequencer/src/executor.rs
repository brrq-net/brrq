...
pub fn execute_transaction_with_context(
    ...
    let mut execute = || -> Result<(), String> {
        ...
        match &tx.body.kind {
            TransactionKind::Transfer { to, amount } => {
                if to.is_zero() {
                    return Err("Transfer to zero address is not allowed".into());
                }
    ...
    // ── 7.5. Priority tip distribution ───────────────────────────────────────────
    // Priority fees are NOT distributed here. They remain "in limbo" after the
    // sender pays effective_gas_price × gas_used and base_fee is recorded as
    // burned. The priority_fee portion is distributed at the BLOCK level by
    // distribute_fees() in block_producer.rs, which splits them according to
    // the protocol spec: 30% sequencer, 40% prover pool, 20% DA reserve,
    // 10% protocol treasury.
    //
    // Distributing tips here AND in distribute_fees would double-count them,
...
> {
    ...
    loop {
        match current_state_res {
            ...
            Ok(brrq_vm::executor::ExecutionState::YieldDelegateCall {
                to: call_to,
                calldata: call_calldata,
            }) => {
                // Delegate call: execute target code but preserve caller and storage context.
                let remaining_gas = gas_available.saturating_sub(vm.gas_used());
                let sub_gas = remaining_gas.saturating_sub(remaining_gas / 64);
                ...
                let (sub_success, sub_gas_used, sub_return_data, mut _sub_logs, sub_trace) =
                    execute_contract_call_internal(
                        state,
                        changes,
                        caller, // preserve original caller (not current contract)
                        &call_to,
                        0, // delegate calls do not transfer value
                        &call_calldata,
                        sub_gas,
                        depth + 1,
                        ctx,
                ...
                if let Err(e) = vm.consume_gas(sub_gas_used) {
                    tracing::warn!("Delegate sub-call exceeded parent gas: {:?}", e);
                }
